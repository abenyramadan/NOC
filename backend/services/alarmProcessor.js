...
export class AlarmProcessor {
  ...
  async sendAlarmEmail(alarm) {
    try {
      ...
      // Get email service instance first
      const emailService = await getEmailService();
      
      // Check for recipients in this order:
      // 1. Recipients attached to the alarm
      // 2. Immediate alerts from email service
      // 3. Fallback to environment variable or default
      const alarmRecipients = alarm.recipients && alarm.recipients.length > 0
        ? (Array.isArray(alarm.recipients) ? alarm.recipients : [alarm.recipients])
        : [];

      const filteredAlarmRecipients = alarmRecipients
        .map((e) => String(e).trim().toLowerCase())
        .filter(Boolean)
        .filter((e) => e !== "noc@example.com");

      const recipients = filteredAlarmRecipients.length > 0
        ? filteredAlarmRecipients
        : emailService.recipients?.immediateAlerts?.length
          ? emailService.recipients.immediateAlerts
          : process.env.NOC_ALERTS_EMAIL?.split(",") || ["noc@example.com"];
  ...
  async createTicket(alarm) {
    try {
      ...
      const ticketData = {
        ...
        emailSubject: `[${alarm.severity.toUpperCase()}] ${alarm.alarmType} - ${
          alarm.siteName
        }`,
        status: "sent",
        priority:
      ...
      console.error(
        `❌ Failed to create ticket for alarm ${alarm._id}:`,
        error.message
      );
      console.error("Error stack:", error.stack);
      throw error;
  ...
  async createOutageReportFromTicket(ticket, alarm) {
    try {
      console.log(
        `📊 [OUTAGE REPORT] Starting outage report creation for ticket ${ticket._id}...`
      );
      ...
      console.log(
        `🔍 [OUTAGE REPORT] Looking up site with ID: ${alarm.siteId}`
      );
      let site = await Site.findOne({ siteId: alarm.siteId });

      if (!site) {
        console.warn(
          `⚠️ [OUTAGE REPORT] No site found with ID: ${alarm.siteId}`
        );
        // Try to find by name as fallback
        const siteByName = await Site.findOne({ name: alarm.siteName });
        if (siteByName) {
          console.log(
            `ℹ️ [OUTAGE REPORT] Found site by name: ${siteByName.siteId}`
          );
          site = siteByName;
        }
      }
      ...
      console.log(
        `🌍 [OUTAGE REPORT] Region: ${actualRegion}, Supervisor: ${actualSupervisor}`
      );
      ...
      const expectedRestorationTime = calculateExpectedRestorationTime(
        alarm.severity,
        alarm.timestamp
      );
      const mandatoryRestorationTime = calculateMandatoryRestorationTime(
        alarm.severity,
        alarm.timestamp
      );
      console.log(
        `⏱️ [OUTAGE REPORT] Expected restoration: ${expectedRestorationTime}`
      );
      console.log(
        `⏱️ [OUTAGE REPORT] Mandatory restoration: ${mandatoryRestorationTime}`
      );
      ...
      console.log(
        `📝 [OUTAGE REPORT] Creating outage report with data:`,
        JSON.stringify(
          {
            ...outageReportData,
      ...
      const savedReport = await outageReport
        .save()
        .then((doc) => {
          console.log(
            `✅ [OUTAGE REPORT] Successfully created outage report:`,
            {
              id: doc._id,
              siteNo: doc.siteNo,
        ...
        .catch((error) => {
          console.error(`❌ [OUTAGE REPORT] Failed to save outage report:`, {
            ...
            errors: error.errors
              ? Object.entries(error.errors).map(([key, err]) => ({
                  field: key,
                  message: err.message,
                  type: err.kind,
                  value: err.value,
      ...
      console.error(
        "❌ [OUTAGE REPORT] Critical error creating outage report:",
        {
          error: error.message,
          stack: error.stack,
          alarmId: alarm?._id,
  ...
  determineRegion(siteName, actualRegion) {
    ...
    if (
      name.includes("juba") ||
      name.includes("yei") ||
      name.includes("terekeka")
    ) {
      return "C.E.S";
    } else if (
      name.includes("bor") ||
      name.includes("pibor") ||
      name.includes("kapoeta")
    ) {
      return "E.E.S";
    } else if (
      name.includes("wau") ||
      ...
      name.includes("aweil") ||
      name.includes("wunrok") ||
      name.includes("kuajok")
    ) {
      return "Bahr gha zal";
    } else if (
      name.includes("bentiu") ||
      name.includes("rubkona") ||
      name.includes("leer")
    ) {
      return "Equatoria";
    }
