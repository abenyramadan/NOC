    ...
    try {
      ...
      for (const rawAlarm of rawAlarms) {
        try {
            ...
            try {
              ...
              if (recipients.length > 0) {
                ...
                if (emailService && emailService.isConfigured) {
                  ...
                  console.log(
                    `📧 Email notification sent for new alarm: ${newAlarm._id}`
                  );
                }
              }
            } catch (emailError) {
              console.error(
                `⚠️ Failed to send email for alarm ${newAlarm._id}:`,
                emailError.message
              );
          ...
          console.error(
            `❌ Error processing alarm ${rawAlarm.id}:`,
            error.message
          );
          errors++;
        }
      ...
      console.log(
        `✅ ${this.name} sync completed: ${processed} new, ${updated} updated, ${errors} errors`
      );
