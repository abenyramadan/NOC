...
import { alarmProcessor } from "./services/alarmProcessor.js";
...
import HuaweiMaeAlarmService from "./services/huaweiMaeAlarmService.js";
...
// Initialize the server in an async function
async function initializeServer() {
  ...
  const corsOptions = {
    ...
    exposedHeaders: ['Content-Range', 'X-Total-Count']  // Expose custom headers
  };
  ...
  app.post("/update-from-email", async (req, res) => {
    try {
      ...
      for (const [key, value] of Object.entries(formData)) {
        if (
          key.startsWith("rootCause_") ||
          key.startsWith("username_") ||
          key.startsWith("resolutionTime_") ||
          key.startsWith("status_")
        ) {
          const reportId = key.split("_")[1];
          console.log(`🔍 Processing field: ${key} = ${value}`);
          console.log(
            `📋 Extracted reportId: '${reportId}' (type: ${typeof reportId}, length: ${
              reportId?.length
            })`
          );
          ...
          if (reportId.length !== 24) {
            console.log(
              `❌ Invalid ObjectId length for ${reportId} (expected 24, got ${reportId.length})`
            );
            errorCount++;
            updates.push({
              reportId,
          ...
          if (Object.keys(updateData).length > 0) {
            try {
              ...
              errorCount++;
              console.log(
                `❌ Error updating report ${reportId}: ${error.message}`
              );
              updates.push({
                reportId,
                field,
      ...
      res.redirect(
        `/update-result.html?error=${encodeURIComponent(error.message)}`
      );
    }
  });
          ...
          <div class="container">
                ...
                req.query.success === "true"
                  ...
                  <div class="success">
                      ✅ <strong>Success!</strong> ${
                        req.query.updated || "Some"
                      } outage report(s) updated successfully.
                  </div>
              `
                  : ""
              }
              ...
              ${
                req.query.error
                  ? `
                  <div class="error">
                      ❌ <strong>Error:</strong> ${decodeURIComponent(
                        req.query.error
                      )}
                  </div>
              `
                ...
                req.query.updated || req.query.errors
                  ...
                  <div style="background-color: #f8fafc; padding: 15px; border-radius: 4px; margin-bottom: 20px;">
                      ...
                      <ul>
                          ${
                            req.query.updated
                              ? `<li style="color: #16a34a;">✅ ${req.query.updated} field(s) updated successfully</li>`
                              : ""
                          }
                          ${
                            req.query.errors
                              ? `<li style="color: #dc2626;">❌ ${req.query.errors} field(s) failed to update</li>`
                              : ""
                          }
                      </ul>
                  </div>
