...
export const getNotificationRules = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error fetching notification rules",
        error: error.message,
      });
  }
};
...
export const createNotificationRule = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({
        message: "Error creating notification rule",
        error: error.message,
      });
  }
};
  ...
  try {
    ...
    if (recipients)
      updateData.recipients = Array.isArray(recipients)
        ...
        : [recipients];
    if (methods)
      updateData.methods = Array.isArray(methods) ? methods : [methods];
    ...
    res
      .status(400)
      .json({
        message: "Error updating notification rule",
        error: error.message,
      });
  }
};
...
export const toggleNotificationRule = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error toggling notification rule",
        error: error.message,
      });
  }
};
